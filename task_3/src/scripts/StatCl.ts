...
export class StatCl implements Stat {
    private statDict: { [key: number]: number } = {};

    log(winAmount: number, hitCount: number) {
        if((winAmount < 0 || winAmount == Infinity) || 
            ...
            // is there any other ways to round except 
            // let winAmountRounded = Number(winAmount.toFixed(1))?
    ...
    merge(anotherStat: StatCl) {
        for (const key of 
            Object.keys(anotherStat.statDict).map((str) => parseFloat(str))) {
    ...
    print() {
        let thisDictKeys =
            Object.keys(this.statDict).map((str) => parseFloat(str)).sort();

        let sumAllWins: number = 0;
        let countAllHits: number = 0;
